...
export interface EventoNocturno {
  ...
  minutosAntesManana: number;
  esSalidaTardia: boolean;
  esEntradaTemprana: boolean;
  tipoRecargo: 'salida_tardia' | 'entrada_temprana' | 'ambos';
}
...
// Badge para hora de ENTRADA — muestra alerta si es antes de las 05:00
const getEntradaBadge = (horaEntrada: string, esEntradaTemprana: boolean, minutosAntesManana: number) => {
  if (!horaEntrada) return <span className="text-gray-400 text-sm">-</span>;
  if (!esEntradaTemprana) {
    return <span className="text-sm font-medium text-gray-900">{horaEntrada}</span>;
  }
  return (
    <div className="flex flex-col items-center gap-0.5">
      <span className="text-sm font-bold text-gray-800">{horaEntrada}</span>
      <span className="text-[10px] px-2 py-0.5 rounded-full font-medium bg-indigo-100 text-indigo-800">
        {minutosAntesManana} min antes 05:00
      </span>
    </div>
  );
};

// Badge para hora de SALIDA — muestra alerta si es después de las 19:00
const getSalidaBadge = (horaSalida: string, esSalidaTardia: boolean, minutosExtrasNoche: number) => {
  if (!horaSalida) return <span className="text-gray-400 text-sm">-</span>;
  if (!esSalidaTardia) {
    return <span className="text-sm font-medium text-gray-900">{horaSalida}</span>;
  }
  return (
    <div className="flex flex-col items-center gap-0.5">
      ...
      <span className="text-[10px] px-2 py-0.5 rounded-full font-medium bg-blue-100 text-blue-800">
        +{minutosExtrasNoche} min desde 20:00
      </span>
    </div>
  );
};

// Badge de tipo de recargo
const getTipoRecargoBadge = (tipo: EventoNocturno['tipoRecargo']) => {
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gradient-to-r from-slate-700 to-slate-800 text-white text-sm">
          <tr>
            ...
            <th className="py-2.5 px-3 text-center font-semibold">Entrada</th>
            ...
            <th className="py-2.5 px-3 text-center font-semibold">Tipo de recargo</th>
            ...
            {paginatedEventos.length === 0 ? (
              <tr>
                <td colSpan={9} className="text-center py-10 text-gray-500">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400">Salida después de 19:00 o entrada antes de 06:00 · Más de 10h trabajadas</span>
                  </div>
                </td>
              </tr>
            ) : (
              paginatedEventos.map((evento, index) => (
                <tr key={index} className="hover:bg-gray-50 transition-colors">
                  ...
                  {/* Entrada — badge especial si es antes de 06:00 */}
                  <td className="px-4 py-4 whitespace-nowrap text-center">
                    {getEntradaBadge(evento.horaEntrada, evento.esEntradaTemprana, evento.minutosAntesManana)}
                  </td>

                  {/* Salida — badge especial si es después de 19:00 */}
                  <td className="px-4 py-4 whitespace-nowrap text-center">
                    {getSalidaBadge(evento.horaSalida, evento.esSalidaTardia, evento.minutosExtrasNoche)}
                  </td>
                  ...
                  {/* Tipo de recargo */}
                  <td className="px-4 py-4 whitespace-nowrap text-center">
                    {getTipoRecargoBadge(evento.tipoRecargo)}
                  </td>
